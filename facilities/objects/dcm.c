...
** Last Update:		$Author: smm $, $Date: 2006-06-10 04:04:03 $
...
#ifdef _MSC_VER
    fd = _open(name, O_RDONLY | O_BINARY);
#else
...
#ifdef _MSC_VER
	fd = _open(mapName, O_RDONLY | O_BINARY);
#else
	...
	* sequenceItem;
	...
	while ((element = LST_Pop(&group->elementList)) != NULL) {
		...
		if (element->element.d.sq != NULL) {
			...
			PRIVATE_OBJECT* privateObject = (PRIVATE_OBJECT *)sequenceItem->object;
			privateObject->fd = -1;
